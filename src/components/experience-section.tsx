  ...
  return (
    <section id="experience" className="min-h-screen flex items-center px-6 lg:px-12 py-24">
      <div className="max-w-4xl w-full">
        <h2 className="text-3xl font-bold mb-16 text-foreground relative inline-block">
          Work experience
        ...
        <div className="space-y-12">
          {experiences.map((exp, index) => (
            <div key={index} className="group">
              <div className="w-10 h-10 rounded-lg bg-card border border-border mb-6 flex items-center justify-center">
                <div className="w-6 h-6 rounded bg-primary/10" />
              </div>

              <h3 className="text-2xl font-bold mb-2 text-foreground italic">{exp.company}</h3>

              <p className="text-sm text-muted-foreground mb-3">
                <span className="font-semibold">{exp.role}</span> ({exp.year})
              </p>

              <p className="text-muted-foreground mb-6 leading-relaxed max-w-2xl">{exp.description}</p>

              <div className="flex flex-wrap gap-3 text-sm text-muted-foreground/60 mb-6">
                ...
                <div className="ml-6 mb-6 space-y-4 border-l border-primary/30 pl-6">
                  {exp.projects.map((project, i) => (
                    <div key={i} className="group/project">
                      <h4 className="text-sm font-semibold text-foreground mb-1 group-hover/project:text-primary transition-colors">
                        {project.name}
                      </h4>
